    ...
    //setSearchTerms grabs the data on keyup and updates the searchterms
        ...
        <section className="container-xxl pb-5 mb-5">
            ...
            <input 
                type="text"
                style={{ height: '2.3rem' }}
                // value={searchTerms ? plantTerms : ""}
                className="input--wide "
                name="plantSearchName"
                onChange={handleInputChange}
                placeholder="Search for an plant... "
                />
                <button className="primary" value="reset" style={{ height: '2.3rem' }} onClick={e => {                    
                    e.preventDefault()
            ...
            <Dropdown.Toggle className="dropDown"variant="warning" id="dropdown-basic">
            Flower Color
            </Dropdown.Toggle>
            <Dropdown.Menu>
            {
            colors.map (c => <Dropdown.Item key={c.color} eventKey={c.color} onSelect={e => {
            getPlantsbyFlowerColor(e.toLowerCase())}}>
            {c.color}</Dropdown.Item>)
            }
            </Dropdown.Menu>
            </Dropdown>
